...
DO $$ DECLARE c text; BEGIN
  ...
  ALTER TABLE deliverable.deliverable_revision ADD CONSTRAINT deliverable_template_version_check CHECK (template_version IN ('analysis-valuation-1.0.0','auction-control-1.0.0','teaser-1.0.0'));
  ...
  ALTER TABLE jobs.job ADD CONSTRAINT job_command_type_check CHECK(command_type IN ('reference_workspace_build','analysis_workbook_build','analysis_workbook_qc','auction_control_workbook_build','teaser_presentation_build','internal_controlled_export','workbook_ai_review')),
    ADD CONSTRAINT job_purpose_code_check CHECK(purpose_code IN ('reference_workspace_build','analysis_workbook_build','analysis_workbook_qc','auction_control_workbook_build','teaser_presentation_build','internal_controlled_export','workbook_ai_review')),
    ADD CONSTRAINT job_allowance_class_check CHECK(allowance_class IN ('reference_workspace_build','analysis_workbook_build','analysis_workbook_qc','auction_control_workbook_build','teaser_presentation_build','internal_controlled_export','workbook_ai_review'));
  ...
  ALTER TABLE deliverable.qc_run ADD CONSTRAINT qc_run_ruleset_check CHECK(ruleset IN ('analysis-workbook-qc-1.0.0','teaser-presentation-qc-1.0.0'));
END $$;
...
ALTER TABLE jobs.job_step ADD CONSTRAINT job_step_operation_class_check CHECK(operation_class IN ('reference_workspace_build','analysis_workbook_build','analysis_workbook_qc','ai_source_proposal','internal_controlled_export','auction_control_workbook_build','teaser_presentation_build'));
ALTER TABLE jobs.job_scope DROP CONSTRAINT job_scope_operation_code_check;
ALTER TABLE jobs.job_scope ADD CONSTRAINT job_scope_operation_code_check CHECK(operation_code IN ('reference_workspace_build','analysis_workbook_build','analysis_workbook_qc','ai_source_proposal','internal_controlled_export','auction_control_workbook_build','teaser_presentation_build'));
ALTER TABLE jobs.job_scope_operation DROP CONSTRAINT job_scope_operation_operation_code_check;
ALTER TABLE jobs.job_scope_operation ADD CONSTRAINT job_scope_operation_operation_code_check CHECK(operation_code IN ('reference_workspace_build','analysis_workbook_build','analysis_workbook_qc','ai_source_proposal','internal_controlled_export','auction_control_workbook_build','teaser_presentation_build'));
...
DO $$ DECLARE c text; BEGIN
 ...
 ALTER TABLE ai.task_definition ADD CONSTRAINT ai_task_definition_code_check CHECK(task_definition IN ('claim_evidence_linking','contract_repair','deliverable_semantic_qc','financial_normalization_mapping','financial_semantic_extraction','material_source_conflict_analysis','native_reader_semantic_parity_review','sell_side_analysis_draft','semantic_change_impact_proposal','source_claim_extraction','teaser_content_draft','valuation_commentary_draft','workbook_commentary_draft'));
 ...
 ALTER TABLE ai.proposal ADD CONSTRAINT ai_proposal_kind_check CHECK(proposal_kind IN ('claim','parity_finding','semantic_qc_finding','teaser_content','workbook_commentary'));
END $$;

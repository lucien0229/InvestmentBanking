...
test -f "${APP_RELEASE_PATH}/services/artifact-signer/server.mjs"
...
export ARTIFACT_SIGNER_PRIVATE_KEY_HOST_PATH="${ARTIFACT_SIGNER_PRIVATE_KEY_HOST_PATH:-/mnt/nvme/ib-artifact-signer-private/artifact-ed25519.pem}"
if getent passwd ib-artifact-sign >/dev/null 2>&1; then
  export ARTIFACT_SIGNER_UID="${ARTIFACT_SIGNER_UID:-$(id -u ib-artifact-sign)}"
  export ARTIFACT_SIGNER_GID="${ARTIFACT_SIGNER_GID:-$(id -g ib-artifact-sign)}"
else
  export ARTIFACT_SIGNER_UID="${ARTIFACT_SIGNER_UID:-993}"
  export ARTIFACT_SIGNER_GID="${ARTIFACT_SIGNER_GID:-979}"
fi
export ARTIFACT_IDENTITY_DIRECTORY="${CELL_ROOT}/shared/artifact-identity"
mkdir -p "${ARTIFACT_IDENTITY_DIRECTORY}"
test -f "${ARTIFACT_SIGNER_PRIVATE_KEY_HOST_PATH}"

...
export default function PreflightPage() {
  ...
  return <main className="dc-page">
    ...
    {result && <section className="dc-surface-card" aria-live="polite"><StatusBadge tone={result.result === "pass" ? "success" : "warning"}>{words(result.result)}</StatusBadge><h2>Saved control results</h2><p>{words(result.reason_code)}</p><p>Recovery: {words(result.recovery_action)}</p><p>Output ceiling: {words(result.output_ceiling ?? "none")}</p><div className="dc-table-wrap"><table><thead><tr><th>Control</th><th>Outcome</th><th>Basis / recovery</th></tr></thead><tbody>{(result.controls ?? []).map((control) => <tr key={control.dimension}><td>{words(control.dimension)}</td><td>{words(control.outcome)}</td><td>{words(control.reason_code)}<br /><small>{words(control.recovery_action)}</small></td></tr>)}</tbody></table></div><div className="dc-grid-two"><div><h3>Permitted scope</h3><ul>{result.permitted_scope.map((scope) => <li key={scope}>{words(scope)}</li>)}</ul></div><div><h3>Excluded scope</h3><ul>{result.excluded_scope.map((scope) => <li key={scope}>{words(scope)}</li>)}</ul></div></div>{result.result === "limited-proceed" && <StatePanel tone="warning" label="Exact limited scope" title={limitedAccepted ? "Limited scope acceptance recorded" : "Accept the displayed limits before continuing"} detail="Only the displayed permitted scope is available. Excluded operations and external use remain blocked.">{!limitedAccepted && <button type="button" className="dc-button dc-material-command" onClick={acceptLimited} disabled={busy}>Accept this exact limited scope</button>}</StatePanel>}{result.result === "pass" ? <div className="dc-page-actions"><a className="dc-button" href={`/app/deals/${dealId}/sources/add`}>Add the first Source</a><a className="dc-button dc-button-secondary" href={`/app/deals/${dealId}/guide`}>Open First Deal Guide</a></div> : result.result === "limited-proceed" && limitedAccepted ? <p><a href={`/app/deals/${dealId}/sources`}>Inspect Sources within the accepted scope →</a></p> : result.result === "limited-proceed" ? null : <StatePanel label="Setup action required" title="Complete the stated declarations before processing" detail="Review Source rights and minimum input readiness in Deal Setup, then run Paid Preflight again." />}</section>}
  </main>;
}

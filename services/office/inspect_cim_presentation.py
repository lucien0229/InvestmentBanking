"""Independent CIM observer: exact slide/page, material text and native shape checks."""
...
def inspect(data, native: Path, reader: Path, render_report=None):
    ...
    try:
        ...
        lineage_ok = all(s.get("evidence_refs") and s.get("source_refs") and s.get("qualification") for s in sections)
        checks.append(check("bidirectional_lineage",lineage_ok,"Every material section has controlled input references, qualification and point-of-use identity in both representations.",{"sections":len(sections)}))
        shape_fail=[]
        ...
        for i, slide in enumerate(prs.slides):
            note=norm(slide.notes_slide.notes_text_frame.text)
            expected=("Control note: proposal-only; do not circulate without exact Review and External-Use Decision." if i==0 else f"Section {sections[i-1]['section_key']} · qualification: {sections[i-1]['qualification']}")
            if note != norm(expected): notes.append({"slide":i+1,"expected":expected,"actual":note})
        checks.append(check("hidden_internal_notes",not notes,"Native notes contain only the declared control/qualification notes and no hidden internal material.",{"notes":notes}))
        if render_report is not None:
            native_sha=hashlib.sha256(native.read_bytes()).hexdigest(); reader_sha=hashlib.sha256(reader.read_bytes()).hexdigest()
            identity=render_report.get("native_reader_identity") or {}; files={f.get("path"):f for f in render_report.get("files",[]) if isinstance(f,dict)}
            report_ok=(render_report.get("revision_id")==data.get("revision_id") and render_report.get("template_version")=="cim-1.0.0" and identity.get("native_sha256")==native_sha and identity.get("reader_sha256")==reader_sha and files.get("cim.pptx",{}).get("sha256")==native_sha and files.get("cim.pdf",{}).get("sha256")==reader_sha and {c.get("code") for c in render_report.get("qc",[])} >= {"native_structure","native_reader_parity","citation_lineage","proposal_only"})
            checks.append(check("render_identity_qc_scope",report_ok,"Render report binds exact hashes, revision/template identity and the required CIM QC scope.",{"native_sha256":native_sha,"reader_sha256":reader_sha}))
    except Exception as exc:
        checks.append(check("native_structure",False,str(exc),{"exception":type(exc).__name__}))
    return {"revision_id":data.get("revision_id"),"checks":checks,"report":{"revision_id":data.get("revision_id"),"template_version":"cim-1.0.0"}}

if __name__=="__main__":
    import sys
    report_path=Path(sys.argv[5]) if len(sys.argv)>5 and Path(sys.argv[5]).exists() else None
    result=inspect(json.loads(Path(sys.argv[1]).read_text()),Path(sys.argv[2]),Path(sys.argv[3]),json.loads(report_path.read_text()) if report_path else None); Path(sys.argv[4]).write_text(json.dumps(result,indent=2))

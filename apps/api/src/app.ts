...
import { registerPreparationPackageRoutes } from "./preparation-package.js";
...
export async function buildApi(options: BuildApiOptions = {}): Promise<FastifyInstance & { database: Database; referenceJobRuntime: ReferenceJobRuntime }> {
  ...
  registerPreparationPackageRoutes(api, database, { requireBanker, commandKey });
  ...
  api.get("/api/v1/account/usage", async (request, reply) => {
    ...
    const result = await database.withContext(session, null, async (client) => {
      ...
      const capacityUsage = await client.query("SELECT app.get_account_capacity_usage(app.policy_account_id(), app.policy_actor_id()) AS used_active_deals");
      return { active_deal_capacity: Number(ent.rows[0].active_deal_capacity), granted_active_deals: Number(rows.rows[0].granted_active_deals), used_active_deals: Number(capacityUsage.rows[0].used_active_deals), granted_allowances: { intensive_logical_pages: Number(rows.rows[0].intensive_logical_pages), intensive_operations: Number(rows.rows[0].intensive_operations), archive_capacity_gb: Number(rows.rows[0].archive_capacity_gb) } };
    });
    if (result.kind === "invalid") return problem(reply, 401, "session_expired", "The session is no longer valid.", "reauthenticate", request.url);
    if (result.kind === "passkey_required") return problem(reply, 403, "passkey_required", "A Passkey-backed session is required.", "register_passkey", request.url);
    if (result.kind === "not_found") return problem(reply, 404, "resource_not_found", "The requested resource is not available.", "return_to_safe_parent", request.url);
  ...
  api.get("/api/v1/account/commerce-state", async (request, reply) => {
    ...
    if (!session) return;
    const result = await database.withContext(session, null, async (client) => {
      const [entitlements, receipts, usage, audit, measurements, capacityUsage] = await Promise.all([
        ...
        client.query("SELECT app.get_account_capacity_usage(app.policy_account_id(), app.policy_actor_id()) AS used_active_deals"),
      ]);
      return { entitlements: entitlements.rows.map((row) => ({ ...row, active_deal_capacity: Number(row.active_deal_capacity) })), receipts: receipts.rows, usage: { active_deal_capacity: Number(usage.rows[0].active_deal_capacity), granted_active_deals: Number(usage.rows[0].granted_active_deals), used_active_deals: Number(capacityUsage.rows[0].used_active_deals), granted_allowances: { intensive_logical_pages: Number(usage.rows[0].intensive_logical_pages), intensive_operations: Number(usage.rows[0].intensive_operations), archive_capacity_gb: Number(usage.rows[0].archive_capacity_gb) } }, audit_events: audit.rows, measurement_candidates: measurements.rows };
    });
    if (result.kind === "invalid") return problem(reply, 401, "session_expired", "The session is no longer valid.", "reauthenticate", request.url);
    if (result.kind === "passkey_required") return problem(reply, 403, "passkey_required", "A Passkey-backed session is required.", "register_passkey", request.url);
    if (result.kind === "not_found") return problem(reply, 404, "resource_not_found", "The requested resource is not available.", "return_to_safe_parent", request.url);

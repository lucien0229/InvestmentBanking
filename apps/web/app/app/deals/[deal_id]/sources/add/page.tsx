...
export default function AddSourcePage() {
  ...
  return <main style={{ maxWidth: 860, margin: "0 auto", padding: 40 }}>
    ...
    <p>Upload is resumable and Deal-scoped. We do not treat a file as a Source Record until quarantine, authority, rights, and classification are explicit.</p>
    <section aria-label="source intake status" style={{ border: "1px solid #d8dee4", borderRadius: 12, padding: 20, background: "#fff", margin: "24px 0" }}>
      <strong>{stateLabel}</strong><p role={state === "rejected" ? "alert" : "status"} style={{ marginBottom: 0 }}>{error || message}</p>
    </section>
    <form onSubmit={addSource} style={{ display: "grid", gap: 16, maxWidth: 620 }}>
      ...
      <p style={{ color: "#4b5563", fontSize: 14 }}>Safety boundary: macros, executable content, unsafe archive paths, malformed packages, and unsupported active content are rejected. External links are not refreshed.</p>
      <button type="submit" disabled={state === "uploading"}>{state === "uploading" ? "Uploading…" : "Upload and review source"}</button>
    </form>
  </main>;
}

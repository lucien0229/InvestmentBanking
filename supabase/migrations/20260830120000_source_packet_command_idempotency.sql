-- Ticket 08: durable command replay for every mutating Source Packet,
-- Work Objective, condition, and rights command.
CREATE TABLE IF NOT EXISTS source.source_packet_command_idempotency (
...
ALTER TABLE source.source_packet_command_idempotency ENABLE ROW LEVEL SECURITY;
ALTER TABLE source.source_packet_command_idempotency FORCE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS source_packet_command_idempotency_scope ON source.source_packet_command_idempotency;
CREATE POLICY source_packet_command_idempotency_scope ON source.source_packet_command_idempotency
...
GRANT SELECT, INSERT, UPDATE ON source.source_packet_command_idempotency TO app_source_owner;
...
BEGIN
  ...
  END IF;
  SELECT * INTO prior
  FROM source.source_packet_command_idempotency
...
CREATE OR REPLACE FUNCTION source.ticket08_command_record(
...
BEGIN
  IF p_account_id IS DISTINCT FROM app.policy_account_id()
    ...
    RAISE EXCEPTION 'source_packet_scope_mismatch' USING ERRCODE = '42501';
  END IF;
  INSERT INTO source.source_packet_command_idempotency(account_id, actor_id, deal_id, command_type, key_hash, request_digest, response, status_code)
...
GRANT EXECUTE ON FUNCTION source.ticket08_command_replay(uuid,uuid,uuid,text,text,text), source.ticket08_command_record(uuid,uuid,uuid,text,text,text,jsonb,integer) TO app_runtime;

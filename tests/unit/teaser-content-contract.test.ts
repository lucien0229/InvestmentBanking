...
import { teaserDraft } from "../../apps/api/src/deliverables.js";
...
test("teaser draft binds citations and references to its closed input perimeter", () => {
  ...
  const valid = { task_definition: "teaser_content_draft", status: "proposal_only", approved_disclosure_set: ["DISC-001"], evidence: [{ id: evidence, source_record_id: source }], facts_assumptions: [{ id: fact, kind: "fact", status: "accepted" }, { id: assumption, kind: "assumption", status: "approved" }], output_ceiling: { max_slides: 2 }, sections: [{ section_key: "overview", title: "Overview", body: "Proposal", qualification: "Proposal-only", citations: ["DISC-001"], evidence_refs: [evidence], fact_refs: [fact], assumption_refs: [assumption], source_refs: [source] }] };
  assert.equal(teaserDraft.parse(valid).sections.length, 1);
  assert.throws(() => teaserDraft.parse({ ...valid, sections: [{ ...valid.sections[0], citations: ["UNAPPROVED"] }] }));
  assert.throws(() => teaserDraft.parse({ ...valid, sections: [{ ...valid.sections[0], source_refs: ["66666666-6666-4666-8666-666666666666"] }] }));
  assert.throws(() => teaserDraft.parse({ ...valid, output_ceiling: { max_slides: 1 } }));
});

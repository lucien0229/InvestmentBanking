...
BEGIN
 ...
 FOR section IN SELECT value FROM jsonb_array_elements(sections) LOOP
  ...
  END LOOP;
 END LOOP;
 RETURN jsonb_build_object('schema_version','1.0.0','task_definition','teaser_content_draft','status','proposal_only','revision_id',p_revision,'deliverable_id',parent.id,'deal_id',parent.deal_id,'template_version','teaser-1.0.0','purpose',parent.purpose,'audience',parent.audience,'confidentiality',parent.confidentiality,'provenance','synthetic','calculations','[]'::jsonb,'evaluation_time',to_char(clock_timestamp() AT TIME ZONE 'UTC','YYYY-MM-DD"T"HH24:MI:SS"Z"'),'limitations',coalesce(p_limitations,'[]'::jsonb),'approved_disclosure_set',p_draft->'approved_disclosure_set','evidence',evidence,'facts_assumptions',basis,'output_ceiling',ceiling,'content_draft',p_draft);
END $$;

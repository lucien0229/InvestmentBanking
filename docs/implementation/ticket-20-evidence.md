...
The remote Supabase migration credential is intentionally not stored on the VPS. `SUPABASE_DB_URL` is already configured in the development GitHub Environment from the earlier database work; no provider credential was copied into the release or runtime. The implementation commit’s `develop` workflow run `34325790531` passed both local migration validation and the hosted development Supabase migration, including the out-of-order reconciliation required by the existing CIM rows.

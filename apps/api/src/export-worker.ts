...
export class ExportRuntime {
  ...
  async runOnce() {
    ...
    try {
      ...
      const built = await buildExportArchive({ exportId: input.export_id, revisionId: input.revision_id, purpose: input.purpose, scope: input.scope, files: members }, this.options.signer ?? signer()); const objectId = crypto.randomUUID(); const encrypted = await encryptProtected(built.archive, "application/zip", objectId);

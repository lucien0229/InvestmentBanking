...
- Authenticated development acceptance was also verified with the Deal-scoped passkey-backed development session; the route loaded the real Deal header (`Workbook development acceptance` / `Preparation`) and the three-card comparison plus Difference ledger. Screenshot: `output/playwright/ticket15-authenticated-correct.png`.
...
The implementation stores metadata, region receipts and lineage in PostgreSQL; file bytes continue to live in the Protected Object Gateway. Per the authorized development alternative, the pinned LibreOffice path is the accepted Office observer for this environment; Microsoft 365/Windows remains a separate production evidence requirement. The authenticated browser run used a temporary development passkey-backed session for acceptance only and is not production Supabase identity evidence.

## Review outcome

The independent review findings were addressed in the hardening migration and API/UI changes: caller-supplied compatibility no longer overrides server unsafe classifications, Human Decisions are bound to the exact conflict and selected disposition, commands persist replay receipts, blocked comparisons remain retryable, accepted region choices are carried into the immutable Revision build input, and the page fetches the Deal-scoped reimport receipt.

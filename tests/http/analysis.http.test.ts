...
test("Analysis API creates a pinned Calculation Version, deterministic Run, and validation receipt", async (t) => {
  ...
  const version = await api.inject({ method: "POST", url: `/api/v1/deals/${dealId}/calculations/${calculationId}/versions`, headers: { cookie, "idempotency-key": key(), "if-match": '"1"' }, payload: { method_code: "ev_to_equity_tie_out", formula_text: "EV + Cash - Debt = Equity", method_version: "2", unit: "USD million", currency: "USD", period: "FY2025E", input_digest: "sha256:pinned-inputs", definition: { measures: [["enterprise_value","100.0"],["cash","6.2"],["debt","10.0"]].map(([measure_key,value_text]) => ({measure_key,definition:measure_key,period:"FY2025E",unit:"USD million",currency:"USD",sign:"positive",precision:1,value_text,source_locator:{kind:"synthetic_uncontrolled_input"},fact_id:null,assumption_id:null,decision_id:null})) } } }); assert.equal(version.statusCode, 201, version.body); const versionId = version.json().data.id as string;

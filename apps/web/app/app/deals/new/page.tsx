...
export default function NewDealPage() {
  ...
  useEffect(() => { if (error) errorSummary.current?.focus(); }, [error, fieldErrors]);
  ...
  function continueStep() {
    ...
    if (step === 0) {
      if (!dealName.trim()) missing["deal-name"] = "Enter a Deal code name.";
      if (!party.trim()) missing.party = "Enter the represented party.";
      if (!subject.trim()) missing.subject = "Enter the transaction subject.";
      if (!inclusions.trim()) missing.inclusions = "Enter at least one included perimeter item.";
    }
    if (step === 2 && !mandate.trim()) missing.mandate = "Describe the mandate objective.";
    if (Object.keys(missing).length) { setFieldErrors(missing); return setError("Complete the required fields to continue."); }
  ...
  return <main className="dc-page">
    ...
    {error && <div ref={errorSummary} role="alert" tabIndex={-1}><strong>{error}</strong>{Object.keys(fieldErrors).length > 0 && <ul>{Object.entries(fieldErrors).map(([id, message]) => <li key={id}><a href={`#${id}`} onClick={(event) => { event.preventDefault(); document.getElementById(id)?.focus(); }}>{message}</a></li>)}</ul>}</div>}
    {status && <p role="status" style={{ color: "#16724b" }}>{status}</p>}
    <form onSubmit={createDeal} className="dc-surface-card dc-setup-form">
      {step === 0 ? <><p role="status">{termsId ? "Completed purchase and authority acknowledgement loaded from your Account." : "Loading your completed purchase…"}</p>{!termsId && <a href="/checkout/order">Continue Checkout →</a>}<div className="dc-setup-form-grid"><label>Deal code name<input id="deal-name" aria-invalid={Boolean(fieldErrors["deal-name"]) } aria-describedby={fieldErrors["deal-name"] ? "deal-name-error" : undefined} value={dealName} onChange={(event) => setDealName(event.target.value)} maxLength={160} required />{fieldErrors["deal-name"] && <small id="deal-name-error">{fieldErrors["deal-name"]}</small>}</label><label>Transaction type<select defaultValue="sell-side"><option value="sell-side">Sell-Side Auction</option></select></label><label>Represented party<input id="party" aria-invalid={Boolean(fieldErrors["party"]) } aria-describedby={fieldErrors["party"] ? "party-error" : undefined} value={party} onChange={(event) => setParty(event.target.value)} maxLength={240} required />{fieldErrors["party"] && <small id="party-error">{fieldErrors["party"]}</small>}</label><label>Transaction subject<input id="subject" aria-invalid={Boolean(fieldErrors["subject"]) } aria-describedby={fieldErrors["subject"] ? "subject-error" : undefined} value={subject} onChange={(event) => setSubject(event.target.value)} maxLength={240} required />{fieldErrors["subject"] && <small id="subject-error">{fieldErrors["subject"]}</small>}</label><label>Included perimeter, one item per line<textarea id="inclusions" aria-invalid={Boolean(fieldErrors["inclusions"]) } aria-describedby={fieldErrors["inclusions"] ? "inclusions-error" : undefined} value={inclusions} onChange={(event) => setInclusions(event.target.value)} required />{fieldErrors["inclusions"] && <small id="inclusions-error">{fieldErrors["inclusions"]}</small>}</label><label>Excluded perimeter, one item per line<textarea value={exclusions} onChange={(event) => setExclusions(event.target.value)} /></label></div></> : null}
      ...
      {step === 2 ? <><label>Mandate objective<textarea id="mandate" aria-invalid={Boolean(fieldErrors["mandate"]) } aria-describedby={fieldErrors["mandate"] ? "mandate-error" : undefined} value={mandate} onChange={(event) => setMandate(event.target.value)} maxLength={500} required />{fieldErrors["mandate"] && <small id="mandate-error">{fieldErrors["mandate"]}</small>}</label><fieldset><legend>Bound the first controlled purpose</legend>{["Establish the first inspectable Source Packet", "Prepare a controlled Analysis & Valuation Workbook", "Create the first Teaser / CIM Revision", "Continue an auction process already in progress"].map((option) => <label className="dc-check-row" key={option}><input type="radio" name="purpose" value={option} checked={purpose === option} onChange={() => setPurpose(option)} /><span><strong>{option}</strong><small>Bounds the first controlled loop without limiting later work areas.</small></span></label>)}</fieldset></> : null}

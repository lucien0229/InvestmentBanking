...
Ticket 07 is resolved for the authorized development boundary. The implementation and deployment evidence are committed in `64e79e9`, `fc0b7d3`, and `6bf33c2`, with no downstream ticket scope advanced.
...
Verification: `npm test` (48 passed), `npm run contracts:check`, `npm exec tsc -- --noEmit`, `npm run web:build`, and `git diff --check` passed against a clean temporary PostgreSQL instance. Playwright CLI reviewed the local Sources page in the unauthenticated safe state. A staged VPS preflight returned API `401` and Web root/Sources `200`; after the controlled switch, `https://dev-banking.aptoren.com/` and the Sources route returned `200`, while the session, template, and Web Evidence API routes returned the expected unauthenticated `401 application/problem+json`. SSH verification confirmed the Ticket 07 symlink and all three services active, with `PROTECTED_STORAGE_ROOT` set to the shared writable path.

SSH access was subsequently verified and a new release was built at `/opt/investmentbanking/releases/20260901-ticket07-dev-v1`, then atomically switched into `/opt/investmentbanking/current`; the prior Ticket 06 release remains preserved for rollback. The migration was applied through the logged-in Supabase SQL Editor in three dependency-ordered chunks after adding the temporary schema-`CREATE` grant required by the non-superuser `postgres` role; a read-only VPS query confirmed 11 Ticket 07 relations, 11 forced-RLS relations, final `app_source_owner` schema CREATE revoked, and sample definer functions owned by `app_source_owner`. Authenticated remote acceptance remains pending because no test Banker session was available. Production-shaped Public Fetch Coordinator/sandbox, DNS-rebinding checks, Supabase TUS/Storage RLS, worker/Outbox quarantine, KMS-backed Protected Object Gateway, downstream async Job execution, and rollback/restore remain explicit follow-up debt and are not claimed as proven here.
...
The former Ticket 07-only UI result is superseded. The unified Ticket 01–07 prototype-based real-browser audit and remediation record is maintained in [`docs/implementation/ticket-01-07-ui-audit.md`](../../../docs/implementation/ticket-01-07-ui-audit.md). The local remediation now serves the Templates / Compatibility route, exposes Web/template entry surfaces, preserves the four-stage Source intake structure, and replaces protected-page blank/indefinite Loading states with explicit recovery panels. This is a local UI implementation result against the confirmed prototype; it does not prove authenticated production/provider behavior or waive the functional development boundary above.

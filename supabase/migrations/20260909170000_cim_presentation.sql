-- Ticket 19: governed CIM Native/Reader loop.
-- Stage-triggered CIM: strict proposal input, editable Native PPTX and exact Reader PDF.
...
CREATE TABLE IF NOT EXISTS deliverable.cim_lineage (
...
ALTER TABLE deliverable.cim_lineage ENABLE ROW LEVEL SECURITY; ALTER TABLE deliverable.cim_lineage FORCE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS cim_lineage_read_scope ON deliverable.cim_lineage; DROP POLICY IF EXISTS cim_lineage_owner ON deliverable.cim_lineage;
CREATE POLICY cim_lineage_read_scope ON deliverable.cim_lineage FOR SELECT TO app_runtime USING(account_id=app.policy_account_id() AND deal_id=app.policy_deal_id());
CREATE POLICY cim_lineage_owner ON deliverable.cim_lineage TO app_deliverable_owner USING(account_id=deliverable.scope_account() AND deal_id=deliverable.scope_deal()) WITH CHECK(account_id=deliverable.scope_account() AND deal_id=deliverable.scope_deal());
GRANT SELECT ON deliverable.cim_lineage TO app_runtime; GRANT SELECT,INSERT ON deliverable.cim_lineage TO app_deliverable_owner;

CREATE OR REPLACE FUNCTION deliverable.create_cim_deliverable(p_key text,p_digest text,p_body jsonb) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog AS $$
...
BEGIN
 PERFORM deliverable.assert_write(); replay:=deliverable.replay('create_cim_deliverable',p_key,p_digest); IF replay IS NOT NULL THEN RETURN replay; END IF;
 INSERT INTO deliverable.deliverable(account_id,deal_id,deliverable_type,title,purpose,audience,confidentiality,stage_applicability,owner_id) VALUES(app.policy_account_id(),app.policy_deal_id(),'cim_presentation',p_body->>'title',p_body->>'purpose',p_body->>'audience',p_body->>'confidentiality',stage,app.policy_actor_id()) RETURNING * INTO row;
 PERFORM app.record_audit('deliverable_created','completed','deliverable',row.id::text,'cim_presentation',gen_random_uuid()::text);
 RETURN deliverable.remember('create_cim_deliverable',p_key,p_digest,to_jsonb(row));
END $$;

CREATE OR REPLACE FUNCTION deliverable.build_cim_input(p_deliverable uuid,p_revision uuid,p_draft jsonb,p_limitations jsonb) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path=deliverable,app,pg_catalog AS $$
DECLARE parent deliverable.deliverable%ROWTYPE; section jsonb; sections jsonb:=p_draft->'sections';
BEGIN
 SELECT * INTO parent FROM deliverable.deliverable WHERE id=p_deliverable AND deliverable_type='cim_presentation'; IF NOT FOUND THEN RAISE EXCEPTION 'artifact_scope_unavailable'; END IF;
 IF jsonb_typeof(p_draft)<>'object' OR p_draft->>'task_definition'<>'cim_content_draft' OR p_draft->>'status'<>'proposal_only' OR jsonb_typeof(sections)<>'array' OR jsonb_array_length(sections) NOT BETWEEN 1 AND 12 THEN RAISE EXCEPTION 'cim_content_contract_invalid'; END IF;
 FOR section IN SELECT value FROM jsonb_array_elements(sections) LOOP
  IF jsonb_typeof(section)<>'object' OR section->>'section_key' IS NULL OR section->>'title' IS NULL OR section->>'body' IS NULL OR jsonb_typeof(section->'citations')<>'array' OR jsonb_array_length(section->'citations')<1 THEN RAISE EXCEPTION 'cim_citation_required'; END IF;
 END LOOP;
 RETURN jsonb_build_object('schema_version','1.0.0','task_definition','cim_content_draft','status','proposal_only','revision_id',p_revision,'deliverable_id',parent.id,'deal_id',parent.deal_id,'template_version','cim-1.0.0','purpose',parent.purpose,'audience',parent.audience,'confidentiality',parent.confidentiality,'evaluation_time',to_char(clock_timestamp() AT TIME ZONE 'UTC','YYYY-MM-DD"T"HH24:MI:SS"Z"'),'limitations',coalesce(p_limitations,'[]'::jsonb),'approved_disclosure_set',coalesce(p_draft->'approved_disclosure_set','[]'::jsonb),'output_ceiling',coalesce(p_draft->'output_ceiling',jsonb_build_object('max_slides',12)),'content_draft',p_draft);
END $$;

CREATE OR REPLACE FUNCTION deliverable.create_cim_revision(p_parent uuid,p_expected_version bigint,p_key text,p_digest text,p_draft jsonb,p_limitations jsonb,p_release text) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog AS $$
...
BEGIN
 PERFORM deliverable.assert_write(); PERFORM pg_advisory_xact_lock(hashtextextended(concat_ws(':',app.policy_account_id(),app.policy_deal_id(),p_parent,'cim_revision'),0));
 SELECT * INTO parent FROM deliverable.deliverable WHERE id=p_parent AND deliverable_type='cim_presentation' FOR UPDATE; IF NOT FOUND THEN RAISE EXCEPTION 'artifact_scope_unavailable'; END IF;
 ...
 IF deliverable.replay('create_cim_revision',p_key,p_digest) IS NOT NULL THEN RETURN deliverable.replay('create_cim_revision',p_key,p_digest); END IF;
 input:=deliverable.build_cim_input(parent.id,revision,p_draft,p_limitations); SELECT coalesce(max(r.ordinal),0)+1 INTO ordinal FROM deliverable.deliverable_revision r WHERE deliverable_id=parent.id;
 INSERT INTO deliverable.deliverable_revision(id,account_id,deal_id,deliverable_id,ordinal,predecessor_id,purpose,audience,confidentiality,template_version,build_input,basis_digest,created_by) VALUES(revision,parent.account_id,parent.deal_id,parent.id,ordinal,parent.current_revision_id,parent.purpose,parent.audience,parent.confidentiality,'cim-1.0.0',input,encode(extensions.digest(input::text,'sha256'),'hex'),app.policy_actor_id());
 INSERT INTO deliverable.cim_lineage(account_id,deal_id,revision_id,section_key,claim_key,citation_refs,source_refs,native_locator,reader_locator)
 ...
 RETURN deliverable.remember('create_cim_revision',p_key,p_digest,jsonb_build_object('id',job,'job_type','cim_presentation_build','state','queued','revision_id',revision,'deliverable_id',parent.id,'row_version',parent.row_version+1));
END $$;
REVOKE ALL ON FUNCTION deliverable.create_cim_deliverable(text,text,jsonb),deliverable.build_cim_input(uuid,uuid,jsonb,jsonb),deliverable.create_cim_revision(uuid,bigint,text,text,jsonb,jsonb,text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION deliverable.create_cim_deliverable(text,text,jsonb),deliverable.create_cim_revision(uuid,bigint,text,text,jsonb,jsonb,text) TO app_runtime;
DO $$ DECLARE c text; BEGIN
...
END $$;
INSERT INTO ai.task_definition(task_definition,task_family,task_definition_version,input_contract_version,output_contract_version,logical_model_role,lifecycle_status,manifest_digest) SELECT 'cim_content_draft','deliverable_content_draft','1.0.0','1.0.0','1.0.0','reasoning_primary','enabled','sha256:1111111111111111111111111111111111111111111111111111111111111111' WHERE NOT EXISTS(SELECT 1 FROM ai.task_definition WHERE task_definition='cim_content_draft');
INSERT INTO ai.prompt_package(task_definition,task_definition_version,package_version,prompt_digest,input_schema_digest,output_schema_digest,context_plan_version,ai_evidence_policy_version,lifecycle_status) VALUES('cim_content_draft','1.0.0','1.0.0','sha256:2222222222222222222222222222222222222222222222222222222222222222','sha256:3333333333333333333333333333333333333333333333333333333333333333','sha256:4444444444444444444444444444444444444444444444444444444444444444','1.0.0','1.0.0','enabled') ON CONFLICT DO NOTHING;
...
BEGIN
 ...
 ELSE
  ...
  END IF;
  INSERT INTO deliverable.qc_run(id,account_id,deal_id,revision_id,job_id,ruleset,checks,report,input_digest) VALUES(qc,row.account_id,row.deal_id,row.revision_id,p_job,CASE WHEN revision.template_version='teaser-1.0.0' THEN 'teaser-presentation-qc-1.0.0' WHEN revision.template_version='cim-1.0.0' THEN 'cim-presentation-qc-1.0.0' ELSE 'analysis-workbook-qc-1.0.0' END,p_checks,p_report,revision.basis_digest);
...
ALTER TABLE deliverable.cim_lineage
...
BEGIN
 ...
 INSERT INTO deliverable.deliverable(account_id,deal_id,deliverable_type,title,purpose,audience,confidentiality,stage_applicability,owner_id,work_objective_id) VALUES(app.policy_account_id(),app.policy_deal_id(),'cim_presentation',p_body->>'title',p_body->>'purpose',p_body->>'audience',p_body->>'confidentiality',stage,app.policy_actor_id(),objective.id) RETURNING * INTO row;
 PERFORM app.record_audit('deliverable_created','completed','deliverable',row.id::text,'cim_presentation',gen_random_uuid()::text);
 RETURN deliverable.remember('create_cim_deliverable',p_key,p_digest,to_jsonb(row)||jsonb_build_object('stage_basis',deal_stage,'work_objective_id',objective.id));
END $$;
...
BEGIN
 SELECT * INTO parent FROM deliverable.deliverable WHERE id=p_deliverable AND deliverable_type='cim_presentation';
 IF NOT FOUND THEN RAISE EXCEPTION 'artifact_scope_unavailable'; END IF;
 IF jsonb_typeof(p_draft)<>'object' OR p_draft->>'task_definition'<>'cim_content_draft' OR p_draft->>'status'<>'proposal_only' OR jsonb_typeof(p_draft->'approved_disclosure_set')<>'array' OR jsonb_array_length(p_draft->'approved_disclosure_set')<1 OR jsonb_typeof(evidence)<>'array' OR jsonb_array_length(evidence)<1 OR jsonb_typeof(basis)<>'array' OR jsonb_array_length(basis)<1 OR jsonb_typeof(ceiling)<>'object' OR (ceiling->>'max_slides')::integer NOT BETWEEN 1 AND 12 OR jsonb_typeof(sections)<>'array' OR jsonb_array_length(sections)<1 OR jsonb_array_length(sections)+1>(ceiling->>'max_slides')::integer THEN RAISE EXCEPTION 'cim_content_contract_invalid'; END IF;
 FOR ref IN SELECT value FROM jsonb_array_elements(evidence) LOOP
  IF jsonb_typeof(ref)<>'object' OR ref->>'id' IS NULL OR ref->>'source_record_id' IS NULL THEN RAISE EXCEPTION 'cim_lineage_required'; END IF;
  IF NOT EXISTS (SELECT 1 FROM knowledge.evidence e WHERE e.id=(ref->>'id')::uuid AND e.account_id=parent.account_id AND e.deal_id=parent.deal_id AND e.source_record_id=(ref->>'source_record_id')::uuid) THEN RAISE EXCEPTION 'cim_evidence_scope_mismatch'; END IF;
 END LOOP;
 FOR ref IN SELECT value FROM jsonb_array_elements(basis) LOOP
  IF jsonb_typeof(ref)<>'object' OR ref->>'id' IS NULL OR ref->>'kind' NOT IN ('fact','assumption') OR ref->>'status' IS NULL THEN RAISE EXCEPTION 'cim_controlled_basis_required'; END IF;
  IF ref->>'kind'='fact' AND NOT EXISTS (SELECT 1 FROM knowledge.fact f WHERE f.id=(ref->>'id')::uuid AND f.account_id=parent.account_id AND f.deal_id=parent.deal_id) THEN RAISE EXCEPTION 'cim_fact_scope_mismatch'; END IF;
  IF ref->>'kind'='assumption' AND NOT EXISTS (SELECT 1 FROM knowledge.assumption a WHERE a.id=(ref->>'id')::uuid AND a.account_id=parent.account_id AND a.deal_id=parent.deal_id) THEN RAISE EXCEPTION 'cim_assumption_scope_mismatch'; END IF;
 END LOOP;
 FOR section IN SELECT value FROM jsonb_array_elements(sections) LOOP
  IF jsonb_typeof(section)<>'object' OR section->>'section_key' IS NULL OR section->>'title' IS NULL OR section->>'body' IS NULL OR section->>'qualification' IS NULL OR jsonb_typeof(section->'citations')<>'array' OR jsonb_array_length(section->'citations')<1 OR jsonb_typeof(section->'evidence_refs')<>'array' OR jsonb_array_length(section->'evidence_refs')<1 OR jsonb_typeof(section->'source_refs')<>'array' OR jsonb_array_length(section->'source_refs')<1 THEN RAISE EXCEPTION 'cim_citation_required'; END IF;
  FOR ref IN SELECT value FROM jsonb_array_elements(section->'citations') LOOP
   IF NOT EXISTS (SELECT 1 FROM jsonb_array_elements(p_draft->'approved_disclosure_set') d WHERE d#>>'{}'=ref#>>'{}') THEN RAISE EXCEPTION 'cim_citation_not_approved'; END IF;
  END LOOP;
  FOR ref IN SELECT value FROM jsonb_array_elements(section->'evidence_refs') LOOP
   IF NOT EXISTS (SELECT 1 FROM jsonb_array_elements(evidence) e WHERE e->>'id'=ref#>>'{}') THEN RAISE EXCEPTION 'cim_lineage_required'; END IF;
  END LOOP;
  FOR ref IN SELECT value FROM jsonb_array_elements(section->'source_refs') LOOP
   IF NOT EXISTS (SELECT 1 FROM jsonb_array_elements(evidence) e WHERE e->>'source_record_id'=ref#>>'{}') THEN RAISE EXCEPTION 'cim_evidence_scope_mismatch'; END IF;
  END LOOP;
  FOR ref IN SELECT value FROM jsonb_array_elements(section->'fact_refs') LOOP
   IF NOT EXISTS (SELECT 1 FROM jsonb_array_elements(basis) e WHERE e->>'id'=ref#>>'{}' AND e->>'kind'='fact') THEN RAISE EXCEPTION 'cim_controlled_basis_required'; END IF;
  END LOOP;
  FOR ref IN SELECT value FROM jsonb_array_elements(section->'assumption_refs') LOOP
   IF NOT EXISTS (SELECT 1 FROM jsonb_array_elements(basis) e WHERE e->>'id'=ref#>>'{}' AND e->>'kind'='assumption') THEN RAISE EXCEPTION 'cim_controlled_basis_required'; END IF;
  END LOOP;
 END LOOP;
 RETURN jsonb_build_object('schema_version','1.0.0','task_definition','cim_content_draft','status','proposal_only','revision_id',p_revision,'deliverable_id',parent.id,'deal_id',parent.deal_id,'template_version','cim-1.0.0','purpose',parent.purpose,'audience',parent.audience,'confidentiality',parent.confidentiality,'provenance','synthetic','calculations','[]'::jsonb,'evaluation_time',to_char(clock_timestamp() AT TIME ZONE 'UTC','YYYY-MM-DD"T"HH24:MI:SS"Z"'),'limitations',coalesce(p_limitations,'[]'::jsonb),'approved_disclosure_set',p_draft->'approved_disclosure_set','evidence',evidence,'facts_assumptions',basis,'output_ceiling',ceiling,'content_draft',p_draft);
END $$;
...
BEGIN
 ...
 input:=deliverable.build_cim_input(parent.id,revision,p_draft,p_limitations)||jsonb_build_object('work_objective_id',objective.id,'packet_version_id',objective.packet_version_id); SELECT coalesce(max(r.ordinal),0)+1 INTO ordinal FROM deliverable.deliverable_revision r WHERE deliverable_id=parent.id;
 ...
 INSERT INTO deliverable.cim_lineage(account_id,deal_id,revision_id,section_key,claim_key,citation_refs,source_refs,native_locator,reader_locator,evidence_refs,fact_refs,assumption_refs,qualification,element_lineage)

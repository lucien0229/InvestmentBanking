...
export function WorkspaceTabs<T extends string>({ label, tabs, value, onChange, children }: { label: string; tabs: readonly T[]; value: T; onChange: (value: T) => void; children: ReactNode }) {
  ...
  }}>{name}</button>)}</div>{tabs.map((name, index) => <div key={name} id={`${id}-panel-${index}`} role="tabpanel" className={index === selected ? "dc-analysis-page" : undefined} aria-labelledby={`${id}-tab-${index}`} hidden={index !== selected} tabIndex={0}>{index === selected ? children : null}</div>)}</>;
}

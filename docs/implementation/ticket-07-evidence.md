Status: `implemented and serving on the authorized development boundary` — Ticket 07 is covered by the local PostgreSQL/RLS and browser-visible Sources seams. The Ticket 07 migration is applied to the Supabase dev branch and the matching release is live on the development VPS. No production-complete claim is made.
...
- Public development probes remain healthy after the controlled switch: `https://dev-banking.aptoren.com/` returned `200`; `/app/deals/00000000-0000-0000-0000-000000000000/sources` returned `200`; `/api/v1/session`, `/api/v1/account/artifact-templates`, and the Deal Web Evidence collection returned the expected unauthenticated `401 application/problem+json`. SSH verification showed `/opt/investmentbanking/current` points to `/opt/investmentbanking/releases/20260901-ticket07-dev-v1`; `investmentbanking-api.service`, `investmentbanking-web.service`, and `nginx` are active; and the API has `PROTECTED_STORAGE_ROOT=/opt/investmentbanking/shared/protected` with the prior Ticket 06 release preserved for rollback. The release passed an isolated staged API/Web preflight before switching (API `401`, Web root/Sources `200`). The migration was applied through the logged-in Supabase SQL Editor in three dependency-ordered chunks after adding the temporary schema-`CREATE` grant required by the non-superuser `postgres` role; a read-only VPS query confirmed 11 Ticket 07 relations, 11 forced-RLS relations, final `app_source_owner` schema CREATE revoked, and sample definer functions owned by `app_source_owner`.

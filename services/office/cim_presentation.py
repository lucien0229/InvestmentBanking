"""Deterministic editable CIM PPTX + exact Reader PDF renderer."""
...
def _validate(data):
    if data.get("schema_version") != "1.0.0" or data.get("template_version") != TEMPLATE:
        raise ValueError("unsupported_cim_contract")
    if data.get("task_definition") != "cim_content_draft" or data.get("status") != "proposal_only":
        raise ValueError("cim_proposal_only_required")
    ...
    if not sections or not isinstance(max_slides, int) or max_slides < 1 or len(sections) + 1 > max_slides:
        raise ValueError("cim_output_ceiling_exceeded")
    if not approved or not evidence or not basis:
        raise ValueError("cim_controlled_inputs_required")
    for section in sections:
        required = ("section_key", "title", "body", "qualification", "citations", "evidence_refs", "source_refs")
        if any(not section.get(k) for k in required): raise ValueError("cim_section_contract_invalid")
        if any(c not in approved for c in section["citations"]): raise ValueError("cim_citation_not_approved")
        if any(e not in evidence for e in section["evidence_refs"]): raise ValueError("cim_evidence_reference_invalid")
        if any(s not in source_records for s in section["source_refs"]): raise ValueError("cim_source_reference_invalid")
        if any(b not in basis or basis[b] != "fact" for b in section.get("fact_refs", [])): raise ValueError("cim_fact_reference_invalid")
        if any(b not in basis or basis[b] != "assumption" for b in section.get("assumption_refs", [])): raise ValueError("cim_assumption_reference_invalid")
...
def build(data, output: Path):
    ...
    prs.core_properties.title = "Deal Control CIM"
    prs.core_properties.subject = f"Revision {data['revision_id']} · proposal-only"
    prs.core_properties.keywords = "cim, proposal-only, controlled disclosure, native-reader parity"
    ...
    _text(slide, "CIM", .65, 1.35, 10, 1, 38, True, (255,255,255))
    ...
    for idx, section in enumerate(sections, 2):
        ...
        _text(slide, "DEAL CONTROL / CIM", .55, .35, 5, .25, 10, True, (84,99,108), "Aptos Mono")
    ...
    native = output / "cim.pptx"; prs.save(native)
    with tempfile.TemporaryDirectory() as tmp:
        office = shutil.which("libreoffice") or shutil.which("soffice") or "libreoffice"
        subprocess.run([office,"--headless","-env:UserInstallation=file:///tmp/cim-lo-profile","--convert-to","pptx","--outdir",tmp,str(native)],check=True,capture_output=True)
        roundtrip = Path(tmp)/"cim.pptx"
        if not roundtrip.exists(): raise ValueError("office_save_reopen_failed")
        shutil.copy(roundtrip,native)
        subprocess.run([office,"--headless","-env:UserInstallation=file:///tmp/cim-lo-profile-pdf","--convert-to","pdf","--outdir",tmp,str(native)],check=True,capture_output=True)
        shutil.copy(Path(tmp)/"cim.pdf",output/"cim.pdf")
        version = subprocess.run([office,"--version"],check=True,text=True,capture_output=True).stdout.strip().splitlines()[0]
    pdf = fitz.open(output/"cim.pdf"); pages=[]

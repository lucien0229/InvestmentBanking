...
The remote Supabase migration credential is intentionally not stored on the VPS. Applying `20260909180000_preparation_package_readiness.sql` to the hosted development branch remains dependent on the configured migration-only `SUPABASE_DB_URL` GitHub Environment secret and its migration workflow; no provider credential was copied into the release or runtime. The latest pushed `develop` workflow run `34322460577` reached `db:migrate` and failed before the hosted migration job, so hosted schema application is still pending and must be rerun after the migration gate failure is resolved.
